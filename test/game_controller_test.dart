  ...
  group('GameController Tests', () {
    ...
    group('1. Secret Word Selection Tests', () {
      test('should select a secret word from the predefined list', () {
        ...
        expect(controller.secretWord.value, isNotEmpty); // Fixed: removed duplicate check
      ...
      test('should not select a secret word that is not in the list', () {
        for (int i = 0; i < 10; i++) {
          controller.newGame();
          expect(controller.words.contains(controller.secretWord.value), true,
              reason: 'Selected word "${controller.secretWord.value}" is not in the list');
        }
      });
    });
    ...
    group('2. Guessing Logic Tests', () {
      setUp(() {
        controller.onInit();
        controller.secretWord.value = 'tree'; // Set a known secret word for testing
      });
      ...
      test('should handle case-insensitive comparisons', () { // Fixed: added missing opening brace
        int initialScore = controller.score.value; // Fixed: 'init' -> 'int'

        controller.makeGuess('TrEe');
        expect(controller.score.value, initialScore + 10); // Fixed: removed extra closing parenthesis
      });
    });
    ...
    group('3. Scoring System Tests', () {
      ...
      test('should accumulate score over multiple guesses', () {
        ...
        controller.makeGuess('star');
        expect(controller.score.value, initialScore + 10); // Score should not change

        controller.secretWord.value = 'star'; // Fixed: need to set new secret word for next correct guess
        controller.makeGuess('star'); // Fixed: this should now be correct
        expect(controller.score.value, initialScore + 20); // Fixed: should be +20, not +10
      });

      test('should maintain score when new game starts', () { // Fixed: added missing opening brace
        controller.makeGuess('tree'); // Make a correct guess first
        int scoreAfterCorrectGuess = controller.score.value; // Fixed: declare this variable
    ...
    group('5. Integration Test', () {
      test('should handle complete game flow correctly', () {
        ...
        expect(controller.guesses.length, 3);
        expect(controller.score.value, initialScore + 10); // Fixed: only one correct guess = +10
        expect(controller.guesses.contains('star'), true); // Fixed: 'book' -> 'star' (matches actual guess)
        expect(controller.guesses.contains('tree'), true);
        expect(controller.guesses.contains('bark'), true);
      });

      test('should reset game state on new game', () { // Fixed: added missing opening brace
        controller.onInit();
        controller.makeGuess('star');
        controller.makeGuess('tree');
        ...
        // Assert
        expect(controller.guesses.length, 0);
        expect(controller.secretWord.value, isNotEmpty); // Fixed: added missing semicolon
        expect(controller.secretWord.value.length, equals(4)); // Fixed: added missing dot
        expect(controller.words.contains(controller.secretWord.value), true);
      });
    ...
    group('6. Error Handling Tests', () { // Fixed: added missing opening brace
      setUp(() {
        controller.onInit();
      });
      ...
      test('should handle null guesses', () {
        ...
        expect(controller.guesses.length, 0, reason: 'Should not accept null guesses');
      });

      test('should handle whitespace-only guesses', () {
        controller.makeGuess('    ');
        expect(controller.guesses.length, 0, reason: 'Should not accept whitespace-only guesses');
      ...
      test('should trim guesses before processing', () {
        ...
        expect(controller.score.value, 10, reason: 'Should accept and correctly process trimmed guesses');
      });
